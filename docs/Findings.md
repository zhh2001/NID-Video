...
### Finding M5-020: K400-transient Bot F1 pattern at ep 0-1 only — 2 of 3 K400 cells show K400 init-effect detection signal eroded by NID fine-tuning [LOW]

- **Context**: K400 transfer mechanism (M5-019 feature-level drift) 在 Bot per-class detection 上的 manifestation 未 isolate;原 m5_5_baselines.md §Findings 3(b) conservative "at most epochs" wording 掩盖了 K400-pretrained cells 早期 transient detection signal。
- **Discovery**: P5 round 1 inventory (4 cells × 10 epochs Bot F1 verbatim) 揭示 6-cell suite 3-tier 结构: K400 sustained 1/6 (R(2+1)D-18 run_avg 0.1404 / ep0 peak 0.2000 / final 0.1429);K400 transient ep 0-1 2/6 (M5.4 P2 main peak 0.1538 @ ep1 final 0;I3D peak 0.1429 @ ep1 final 0);random strict zero 2/6 (C3D-Small + ConvLSTM 0/10);random late non-zero 1/6 (TimeSformer-S R1 final 0.0909 — random + head_lr ×1 不同 mechanism)。
- **Evidence**: outputs/run_20260502_184512 / run_20260503_180604 / run_20260503_202832 / run_20260504_051632(R(2+1)D)/ run_20260504_015958(I3D)各 metrics/per_epoch.json;P5 round 1 forensic 在 outputs/n3_review_p5_bot_f1_inventory.txt(gitignored,local-only)。
- **Status**: open finding, 3-tier pattern anchored at 4-cell scope (6-cell with R(2+1)D-18 + TimeSformer-S R1 refs);K400 init-effect detection-level erosion mechanism(M5-019 feature-level companion);paper-value annotation deferred to design layer

> 🎯 **论文价值标注**
> - **Section**: Methods §3.4 K400 pretrain rationale + Discussion §6.2 K400 transfer mechanism
> - **Use**: Detection-level companion to M5-019(K400 stem feature drift +49% magnitude over 10 epochs)。2 of 3 K400 cells(M5.4 P2 + I3D)Bot detection signal 仅 ep 0-1 active,then erased by NID fine-tuning by ep 2+。R(2+1)D-18 sustained 是 within-K400-group architecture exception 不是 paradigm rule。Combined narrative:K400 transfer = initialization-effect at both detection level (本 finding) + feature level (M5-019),fine-tuning 主动 adapt features away from K400 origin。Paper §6.2 K400 transfer mechanism 双 anchor(detection + feature)。
> - **Quote candidate**: "Among 3 K400-pretrained baselines, R(2+1)D-18 alone sustains Bot per-class F1 > 0 across all 10 epochs (run-average 0.1404). The 2 other K400-pretrained baselines (M5.4 P2 main, I3D) show non-zero Bot F1 only at epochs 0–1 (peaks 0.1538 / 0.1429 at ep1), then collapse to F1 = 0 for ep 2–9 — K400 init-effect detection signal eroded within 2 epochs of NID fine-tuning. This complements M5-019's +49% K400 stem feature drift across 10 epochs."
> - **Risk if missed**: Paper §6.2 K400 transfer mechanism narrative 缺 detection-level evidence;"R(2+1)D-18 唯一 sustained Bot F1" 易 mis-read 为 architecture-bound,实为 K400 init-effect 在 R(2+1)D-18 preserved 较长 within-K400-group exception。
> - **Cross-link**: M5-007 sub(a)(R(2+1)D-18 sustained Bot F1 narrative anchor)/ M5-008(K400 = 5-epoch loss head start)/ M5-019(K400 stem feature drift +49% feature-level companion)
> - **Priority**: LOW
