...
This is the one-off script that produces the m5_3_rerun/ artefact set
attached to the M5.2 production run, removing the round_robin cycling
noise that distorted M5.2's per-epoch metric trajectory. Numbers from
this script are the noise-free baseline against which later loss-function
work is compared.

Combined metrics are what an eval-only run of scripts/train.py
...
Usage:
    uv run python scripts/m5_3_rerun.py \\
        ...
        --output-dir outputs/run_<ts>/m5_3_rerun
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--source-train-macro-f1", type=float, default=None,
                   help="Reference macro_f1 from the source training run "
                        "(e.g. M5.2 best-epoch eval); written into the JSON "
                        "as a delta anchor.")
...
def _render_readme(args: argparse.Namespace, payload: dict) -> str:
    return f"""# Re-eval baseline (M5.3 no_cycle strategy)
...
uv run python scripts/m5_3_rerun.py \\
    ...
    --shard-pattern-slow "{payload['shard_pattern_slow']}" \\
    --splits-path {payload['splits_path']} \\
    --output-dir {Path(payload['ckpt_source']).parent.parent / 'm5_3_rerun'}
```
